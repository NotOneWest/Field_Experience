...
int adv_val[16], x=0, adv, adno, filter, ADV[2]={0, }, NOTCH[2];
void msecint(){
	static int graph_num, adv_max, adv_min, filter_max, filter_min;
	HAL_ADC_Start(&hadc1);
	adv = HAL_ADC_GetValue(&hadc1);
	filter=adv;
	for(int i=0; i<16; i++) filter+=adv_val[i];
	adno = (adno<15) ? adno+1 : 0;
	adv_val[adno]=adv;
	...
	if(graph_num >= 20){
		...
		NOTCH[0] = filter_max; NOTCH[1] = filter_min;

		adv_max=adv_min=adv; filter_max=filter_min=filter;
		graph_num=0;
	}
}
...
void lcd_main(void){
	...
	while(1){
		if(ADV[0]){
			x = (x<480) ? x+1 : 0;
			Line(x, 0, x, 320, BLACK);

			Line(x, NOTCH[1]/150, x, NOTCH[0]/150, GREEN);
			Line(x, (ADV[1]*17-15000)/200, x, (ADV[0]*17-15000)/200, YELLOW);
			ADV[0]=0;
		}
	}
}
