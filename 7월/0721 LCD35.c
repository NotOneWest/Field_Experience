...
int pute8X16(int ch){
	...
	if(gx+16<MAX_Y) {gx+=8;} else {gx=0; gy=gy+16;}
	return ch;
}
...
int x=0, adv, adv_no=0, filter, diff, notch_no=0, flag=0, heartbeat;
...
void msecint(){
	static int graph_count, timer_count, adv_max, adv_min, filter_max, filter_min, diff_max, diff_min;
	HAL_ADC_Start(&hadc1);
	...
	if(MAX < (diff/200) && ((diff/200) > cut)) {
		if(flag && timer_count) heartbeat = (60000/timer_count);
		timer_count=0; MAX=(diff/200); cut=MAX;
	}
	if(timer_count == T_MIN) { MAX=0; cut/=2; flag=1;}
	if(timer_count > T_MAX) { timer_count=0; MAX=0; cut=0; flag=0; }
	timer_count++;
...
void lcd_main(void){
	...
	while(1){
		if(ADV[0]){
			x = (x<480) ? x+1 : 0;
			if(x<24 && heartbeat) LineY(x, 16, 320, BLACK);
			else LineY(x, 0, 320, BLACK);

			LineY(x, MAX_Y-(FILTER[1]/200), MAX_Y-(FILTER[0]/200), GREEN);
...
//			LineY(x, (ADV[1]*17-15000)/200, (ADV[0]*17-15000)/200, YELLOW);
			...
			if(50<=heartbeat && heartbeat<=200){
				pute8X16(heartbeat/100+'0');
				pute8X16((heartbeat/10)%10+'0');
				pute8X16(heartbeat%10+'0');
				gx=0;
			}
			ADV[0]=0;
		}
